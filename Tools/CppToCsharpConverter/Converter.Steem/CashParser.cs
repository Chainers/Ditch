...
namespace Converter.Steem
{
    public static class CashParser
    {
        ...
        private static readonly Regex ApiMethod = new Regex(@"(?<=\()[a-z_]*(?=\))");

        public static IEnumerable<ParsedClass> FindAndParse(SearchTask searchTask, string[] extensions, bool isApi)
        {
            ...
            var lines = Grabber.FindClass(searchTask, extensions);
        ...
        private static bool GoToDeclareApiSection(IList<string> lines, int index, out int end)
        {
            end = index;
            for (var i = index; i < lines.Count; i++)
            {
                if (lines[i].Contains("DECLARE_API"))
                {
                    end = i + 1;
                    return true;
                }
        ...
        private static PreParsedElement TryParseMethod(IList<string> lines, int index, out int end)
        {
            end = index + 1;
