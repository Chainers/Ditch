...
namespace Ditch.Steem.Old.Tests
{
    ...
    public class OperationsTest : BaseTest
    {
        ...
        public void PostWithBeneficiariesTest()
        {
            ...
            var op2 = new BeneficiariesOperation(user.Login, op.Permlink, new Asset(1000000000, 3, "SBD"), new Beneficiary("steepshot", 1000));
        ...
        {
            var op = new TransferOperation(User.Login, User.Login, new Asset("0.001 SBD"), "Hi, it`s test transfer from Ditch (https://github.com/Chainers/Ditch).");
        ...
        {
            var op = new TransferToVestingOperation(User.Login, User.Login, new Asset("0.001 STEEM"));
            var response = Post(User.ActiveKeys, false, op);
            Assert.IsFalse(response.IsError, response.GetErrorMessage());
        }
        ...
        {
            var op = new WithdrawVestingOperation(User.Login, new Asset(1, 6, "VESTS"));
            var response = Post(User.ActiveKeys, false, op);
            Assert.IsFalse(response.IsError, response.GetErrorMessage());
        }
        ...
        //{
        //    var op = new WitnessUpdateOperation(User.Login, "https://golos.io/ru--golos/@steepshot/steepshot-zapuskaet-delegatskuyu-nodu",
        //        new PublicKeyType("GLS1111111111111111111111111111111114T1Anm"), 
        //        new ChainProperties(1000, new Asset("1.000 STEEM"), 131072), new Asset("0.000 STEEM"));
        //    var response = Post(User.ActiveKeys, false, op);
        //    Assert.IsFalse(response.IsError, response.GetErrorMessage());
        //}
        ...
        public void FollowUnfollowTest()
        {
            ...
            var isFollow = false; //IsFollow(autor);
